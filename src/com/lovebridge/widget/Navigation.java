...
public class Navigation extends LinearLayout {
    ...
    public void initView(final Context context, AttributeSet attrs) {
        ...
        int navigation_image_right = typedArray.getResourceId(R.styleable.navigation_image_right, -1);
        if (navigation_image_right != -1) {
            iv_navigation_right.setVisibility(VISIBLE);
            iv_navigation_right.setImageResource(navigation_image_right);
        } else {
            iv_navigation_right.setVisibility(GONE);
        }
    ...
    public void setNavigationTitle(CharSequence title
        ...
        if (tv_navigation_title != null) {
            tv_navigation_title.setText(title);
