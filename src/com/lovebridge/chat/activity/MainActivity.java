...
public class MainActivity extends YARActivity implements EmojiPickerFragment.Listener, ChatTabEntry.Listener, FooterTabLayout.Listener,
    ...
    private String makeFragmentName(long id) {
        return "android:switcher:" + toChatUsername + id;
    }
    ...
    private void handleNotificationInfo() {
        ...
        if ((intent.getFlags() & 0x100000) == 0) {
            String toChatUsername = intent.getStringExtra("userId");
            tabs.setDefaultTab(toChatUsername);
        }
    }
    ...
    private void openChat(ChatTabEntry chatTabEntry, boolean showKeyboard) {
        ...
        String toChatUsername = chatTabEntry.getEMConversation().getUserName();
        if (toChatUsername != null && toChatUsername.equals(this.toChatUsername)) {
            return;
        }
        ...
        initFragment1(ChatFragment.class, makeFragmentName(chatTabEntry.threadId), bundle);
    }
    ...
    private void initFragment(Class<?> name, String tag, Bundle bundle) {
        ...
        YARFragment fragment1 = (YARFragment) fragmentManager.findFragmentByTag(preTag);
    ...
    private void initFragment1(Class<?> name, String tag, Bundle bundle) {
        FragmentManager fragmentManager = getSupportFragmentManager();
        FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
        YARFragment fragment = (YARFragment) Fragment.instantiate(mActivity, name.getName(), bundle);
    ...
    public YARFragment getCurrentFragment() {
        FragmentManager fragmentManager = getSupportFragmentManager();
        YARFragment fragment = (YARFragment) fragmentManager.findFragmentByTag(preTag);
        return fragment;
    }
    ...
    public class NewMessageBroadcastReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {  // 记得把广播给终结掉
            abortBroadcast();
