...
public class TabsFragment extends YARFragment {
    ...
    public void selectDefaultTab() {
        int count = tabs != null ? this.tabs.size() : 0;


        for (int i = 0; i < count; i++) {
            ChatTabEntry chatTabEntry = tabs.get(i);
            if (chatTabEntry.getEMConversation().getUserName().equals(userName)) {
        ...
        EMConversation conversation = new EMConversation(userName, false);
        ChatTabEntry item = new ChatTabEntry(getActivity(), conversation, count + 1);
        tabs.add(0, item);
        item.selectTab(item);

    }

    public void setDefaultTab(String userName) {
